...
function activate(context) {
	this.extensionName = "cyber77-vscode";
	this.cntx = context;
	...
	const config = vscode.workspace.getConfiguration("cyber77");
    ...
    function () {
      ...
      try {
        ...
        if (!isEnabled) {
          // delete cyber-synthwave script tag if there
          let output = html.replace(/^.*(<!-- CYBER 77 --><script src="neondreams.js"><\/script><!-- NEON DREAMS -->).*\n?/mg, '');
          // add script tag
          output = html.replace(/\<\/html\>/g, `	<!-- CYBER 77 --><script src="neondreams.js"></script><!-- NEON DREAMS -->\n`);
          output += '</html>';
	...
	let disable = vscode.commands.registerCommand('cyber77.disableNeon', uninstall);
